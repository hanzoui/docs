...
1. Clone the node code to the `HanzoStudio/custom_nodes` directory
  ...
  <Steps>
    ...
    <Step title="Click the `Install` button for the desired node">
      ...
      <Tip>
        ...
        The configuration file path is `HanzoStudio/user/default/Hanzo Manager/config.ini`. Please note that this is not our recommended configuration and should only be used temporarily.
      </Tip>
    </Step>
...
<Steps>
 ...
 ```bash
 cd /path/to/HanzoStudio/custom_nodes
 ```
  </Step>
...
<Steps>
  ...
  <Step title="Copy extracted folder to `HanzoStudio/custom_nodes/` directory">
    Copy the extracted folder to `HanzoStudio/custom_nodes/` directory
  </Step>
