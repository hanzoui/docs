---
title: "How to Troubleshoot and Solve Hanzo Studio Issues"
...
Here is the overall approach for troubleshooting custom node issues:
 ...
 flowchart TD
    ...
    E -- Yes --> F[<a href="#1-troubleshooting-the-custom-nodes’-frontend-extensions">Troubleshoot in Hanzo Studio frontend</a><br/><li>Only need reload the frontend</li>]
    E -- No --> G[<a href="#2-general-custom-node-troubleshooting">Use general binary search method</a><li>Need to restart Hanzo Studio multiple times</li>]
...
Start Hanzo Desktop with custom nodes disabled from the settings menu
...
<Tabs>
   ...
   5. Double-click the file to run it. If everything is normal, you should see Hanzo Studio start and custom nodes disabled
   </Tab>
   ...
   4. Enter the following command to start Hanzo Studio through the portable python and disable custom nodes
   ```
...
flowchart TD
   ...
   C --> D[Restart Hanzo Studio and test]
...
<Tabs>
   ...
   For custom nodes, we can prioritize troubleshooting those with frontend extensions, as they cause the most issues. Their main conflicts arise from incompatibilities with Hanzo Studio frontend version updates.
   
   Common issues include:
   - Workflows not executing
   - Some nodes can't show preview images(such as save image node)
   - Misaligned UI elements
   - Unable to access Hanzo Studio frontend
   - Completely broken UI or blank screen
   - Unable to communicate normally with Hanzo Studio backend
   ...
   - Users updating Hanzo Studio without synchronously upgrading custom nodes, even though authors have released compatible versions
   - Authors stopping maintenance, leading to incompatibility between custom node extensions and the Hanzo Studio frontend
   ...
   - Hanzo Studio crashes or fails to start
   - And more

   Common causes for these errors:
   - Custom nodes requiring additional wheels like Hanzo Studio-Nunchaku
...
Among these two different types of custom node issues, conflicts between custom node frontend extensions and Hanzo Studio are more common. We'll prioritize troubleshooting these nodes first. Here's the overall troubleshooting approach:
...
<Steps>
   ...
   After starting Hanzo Studio, find the `Extensions` menu in settings and follow the steps shown in the image to disable all third-party extensions
   <Tip>
      If you can't enter Hanzo Studio frontend, just skip the frontend extensions troubleshooting section and continue to [General Custom Node Troubleshooting Approach](#2-general-custom-node-troubleshooting-approach)
   </Tip>
   </Step>
   <Step title="Restart Hanzo Studio">
   After disabling frontend extensions for the first time, it's recommended to restart Hanzo Studio to ensure all frontend extensions are properly disabled
...
Using this method, you don't need to restart Hanzo Studio multiple times - just reload Hanzo Studio after enabling/disabling custom node frontend extensions. Plus, your troubleshooting scope is limited to nodes with frontend extensions, which greatly narrows down the search range.
...
<Steps>
  <Step title="Use Binary Search to Locate Custom Nodes">
      For the binary search localization method, in addition to manual search, we also have automated binary search using hanzo-cli, as detailed below:
      ...
      <Tabs>
         ...
         If you have [Comfy CLI](/hanzo-cli/getting-started) installed, you can use the automated bisect tool to find the problematic node:
         ...
         # Follow the prompts:
         # - Test Hanzo Studio with the current set of enabled nodes
         # - Mark as 'good' if the issue is gone: hanzo-cli node bisect good
         # - Mark as 'bad' if the issue persists: hanzo-cli node bisect bad
         ...
         <Steps>
            ...
            Before starting, enter the `<YOUR_COMFYUI_FOLDER>\Hanzo Studio\` folder
                  <Tabs>
                     <Tab title="Windows">
                     ...
                     # Backup all content
                     cp -r /content/Hanzo Studio/custom_nodes/* /content/custom_nodes_backup/
                     ```
                     </Tab>
                  </Tabs>
               </Step>
               <Step title="List all custom nodes">
                   <Tabs>
                     ...
                     <Tab title="Cloud/Colab">
                     ```bash
                     ls /content/Hanzo Studio/custom_nodes/
                     ```
                     </Tab>
                  </Tabs>
               </Step>
               ...
               Let's assume that you have 8 custom nodes. Move the first half to temporary storage:
                  ...
                  ```bash
                  # Move first half (nodes 1-4) to temp
                  mv /content/Hanzo Studio/custom_nodes/node1 /content/custom_nodes_temp/
                  mv /content/Hanzo Studio/custom_nodes/node2 /content/custom_nodes_temp/
                  mv /content/Hanzo Studio/custom_nodes/node3 /content/custom_nodes_temp/
                  mv /content/Hanzo Studio/custom_nodes/node4 /content/custom_nodes_temp/
                  ```
                  </Tab>
                  </Tabs>
               </Step>
               <Step title="Test Hanzo Studio">
               Start Hanzo Studio normally
                  ```bash
                  python main.py
                  ```
               </Step>
...
1. Remove the problematic node from `custom_nodes/` or disable it in the Hanzo Manager interface
2. Restart Hanzo Studio
...
- Create an issue with your Hanzo Studio version, error messages, reproduction steps, and OS
- Check the node's documentation and Issues page for known issues

### For Hanzo Studio Core Issues
- **GitHub**: [Hanzo Studio Issues](https://github.com/hanzoai/studio/issues)
- **Forum**: [Official Hanzo Studio Forum](https://forum.hanzo.ai/)

### For Desktop App Issues  
- **GitHub**: [Hanzo Desktop Issues](https://github.com/hanzoui/desktop/issues)

### For Frontend Issues
- **GitHub**: [Hanzo Frontend Issues](https://github.com/hanzoui/studio_frontend/issues)
