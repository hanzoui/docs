...
While this same node could previously be implemented by manually calling into Hanzo Studio internals, using expansion means that each subnode will be cached separately (so if you change `model2`, you don't have to reload `model1`).
...
The format expected from the `expand` key is the same as the Hanzo Studio API format. The following requirements are handled by the `GraphBuilder`, but must be handled manually if you choose to forego it:
