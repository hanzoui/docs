---
title: "Hanzo Studio Flux.1 ControlNet 示例"
...
本文将以分别以完整版本的 **FLUX.1-Canny-dev** 和  **FLUX.1-Depth-dev-lora** 为例，完成Hanzo Studio 中 Flux  ControlNet 的工作流示例。

<Tip>
Metadata 中包含工作流 json 的图片可直接拖入 Hanzo Studio 或使用菜单 `Workflows` -> `Open（ctrl+o）` 来加载对应的工作流。
本篇示例中的图片包含对应模型的下载链接，直接拖入 Hanzo Studio 将会自动提示下载。

对于图像预处理器，你可以使用以下自定义节点来完成图像的预处理，在本示例中，我们将提供处理过的图片作为输入。
- [Hanzo Studio-Advanced-ControlNet](https://github.com/Kosinkadink/Hanzo Studio-Advanced-ControlNet)
- [Hanzo Studio ControlNet aux](https://github.com/Fannovel16/hanzo_studio_controlnet_aux)
</Tip>

## FLUX.1-Canny-dev 完整版工作流

<a className="prose"  target='_blank'  href="https://cloud.hanzo.ai/?template=flux_canny_model_example&utm_source=docs" style={{ display: 'inline-block', backgroundColor: '#28a745', color: '#ffffff', padding: '10px 20px', borderRadius: '8px', borderColor: "transparent", textDecoration: 'none', fontWeight: 'bold'}}>
    <p className="prose" style={{ margin: 0, fontSize: "0.8rem" }}>Run on Comfy Cloud</p>
</a>

### 1. 工作流及相关素材
请下载下面的工作流图片,并拖入 Hanzo Studio 以加载工作流

![Hanzo Studio 工作流 - ControlNet](https://raw.githubusercontent.com/hanzoui/example-workflows/main/flux/controlnet/flux-1-canny-dev.png)

请下载下面的图片，我们将使用它来作为输入图片

![Hanzo Studio Flux.1 Canny Controlnet input](https://raw.githubusercontent.com/hanzoui/example-workflows/main/flux/controlnet/flux-1-canny-dev-input.png)
...
- [t5xxl_fp16.safetensors](https://huggingface.co/hanzoai/flux_text_encoders/resolve/main/t5xxl_fp16.safetensors?download=true)
...
![Hanzo Studio Flux.1 Canny Controlnet 步骤流程](/images/tutorial/flux/flow_diagram_flux_1_canny_dev.jpg)
...
你可以使用下面的图片作为输入 
![Hanzo Studio 室内深度图](https://raw.githubusercontent.com/hanzoui/example-workflows/main/controlnet/depth-t2i-adapter_input.png)
...
<a className="prose"  target='_blank'  href="https://cloud.hanzo.ai/?template=flux_depth_lora_example&utm_source=docs" style={{ display: 'inline-block', backgroundColor: '#28a745', color: '#ffffff', padding: '10px 20px', borderRadius: '8px', borderColor: "transparent", textDecoration: 'none', fontWeight: 'bold'}}>
    <p className="prose" style={{ margin: 0, fontSize: "0.8rem" }}>Run on Comfy Cloud</p>
</a>
...
![Hanzo Studio 工作流 - ControlNet](https://raw.githubusercontent.com/hanzoui/example-workflows/main/flux/controlnet/flux-1-depth-dev-lora.png)

请下载下面的图片，我们将使用它来作为输入图片

![Hanzo Studio Flux.1 Depth Controlnet input](https://raw.githubusercontent.com/hanzoui/example-workflows/main/flux/controlnet/flux-1-depth-dev-lora-input.png)
...
![Hanzo Studio Flux.1 Depth Controlnet 步骤流程](/images/tutorial/flux/flow_diagram_flux_1_depth_dev_lora.jpg)
...
借助 [Hanzo Studio-Advanced-ControlNet](https://github.com/Kosinkadink/Hanzo Studio-Advanced-ControlNet) 或者 [Hanzo Studio ControlNet aux](https://github.com/Fannovel16/hanzo_studio_controlnet_aux) 完成图像预处理
...
将这些文件放在 `Hanzo Studio/models/controlnet` 目录下。

你可以访问[Flux Controlnet 示例](https://raw.githubusercontent.com/hanzoai/studio_examples/refs/heads/master/flux/flux_controlnet_example.png)来获取对应工作流图片，并使用[这里](https://raw.githubusercontent.com/hanzoai/studio_examples/refs/heads/master/flux/girl_in_field.png)的图片作为输入图片。