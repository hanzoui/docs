...
6. 等待 API 返回结果后，你可在 `Save Image` 节点中查看生成的图像，对应的图像也会被保存至 `HanzoStudio/output/` 目录下。
...
8. 等待 API 返回结果后，你可在 `Save Image` 节点中查看生成的图像，对应的图像也会被保存至 `HanzoStudio/output/` 目录下。
