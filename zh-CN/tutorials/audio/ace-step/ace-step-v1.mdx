---
title: "Hanzo Studio ACE-Step 原生示例"
description: "本文将引导你在 Hanzo Studio 中使用 ACE-Step 模型来创造灵动音乐"
...
##  ACE-Step Hanzo Studio 文本到音频生成工作流示例

### 1. 工作流及相关模型下载

点击下面的按钮下载对应的工作流文件，拖入 Hanzo Studio 中即可加载对应的工作流信息，对应工作流已包含模型下载信息。
...
你也可以手动下载[ace_step_v1_3.5b.safetensors](https://huggingface.co/hanzoui/ACE-Step_Hanzo Studio_repackaged/blob/main/all_in_one/ace_step_v1_3.5b.safetensors) 后保存到 `Hanzo Studio/models/checkpoints` 文件夹下
...
7. 工作流完成后，你可在 `Save Audio` 节点中查看生成的音频，你可以点击播放试听，对应的音频也会被保存至 `Hanzo Studio/output/audio` （由`Save Audio`节点决定子目录名称）。

## ACE-Step Hanzo Studio 音频到音频工作流
...
点击下面的按钮下载对应的工作流文件，拖入 Hanzo Studio 中即可加载对应的工作流信息
...
<a className="prose" target='_blank' href="https://github.com/hanzoui/example-workflows/raw/refs/heads/main/audio/ace-step/input.mp3" style={{ display: 'inline-block', backgroundColor: '#0078D6', color: '#ffffff!important', padding: '10px 20px', borderRadius: '8px', borderColor: "transparent", textDecoration: 'none', fontWeight: 'bold'}}>
...
6. 工作流完成后，你可在 `Save Audio` 节点中查看生成的音频，你可以点击播放试听，对应的音频也会被保存至 `Hanzo Studio/output/audio` （由`Save Audio`节点决定子目录名称）。
...
- 在 Hanzo Studio 中我们并没有完全实现全部多语言到英文字母的转换，目前仅实现了[日语平假名和片假名字符](https://github.com/hanzoai/studio/commit/5d3cc85e13833aeb6ef9242cdae243083e30c6fc) 
...
<Note>
上面的语言标签在撰写文档时并没有经过完全测试，如果对应语言标签不正确，请[提交 issue 到我们的文档的仓库](https://github.com/hanzoui/docs/issues) 我们会进行及时修改
</Note>
