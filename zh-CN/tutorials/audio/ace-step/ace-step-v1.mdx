...
你也可以手动下载[ace_step_v1_3.5b.safetensors](https://huggingface.co/hanzoui/ACE-Step_HanzoStudio_repackaged/blob/main/all_in_one/ace_step_v1_3.5b.safetensors) 后保存到 `HanzoStudio/models/checkpoints` 文件夹下
...
7. 工作流完成后，你可在 `Save Audio` 节点中查看生成的音频，你可以点击播放试听，对应的音频也会被保存至 `HanzoStudio/output/audio` （由`Save Audio`节点决定子目录名称）。
...
6. 工作流完成后，你可在 `Save Audio` 节点中查看生成的音频，你可以点击播放试听，对应的音频也会被保存至 `HanzoStudio/output/audio` （由`Save Audio`节点决定子目录名称）。
