---
title: "Hanzo Studio ControlNet 使用示例"
description: "本篇将引导了解基础的 ControlNet 概念，并在 Hanzo Studio 中完成对应的图像生成"
...
![Hanzo Studio 随机种子生成的图片](/images/tutorial/controlnet/generated_with_random_seed.jpg)

但随着 ControlNet 的出现，我们可以通过引入额外的条件，来控制图像的生成，比如我们可以使用一张简单的涂鸦，来控制图像的生成，就可以生成差不多类似的图片。

![Hanzo Studio 涂鸦控制图像生成](/images/tutorial/controlnet/scribble_example.jpg)

在本示例中，我们将引导你完成在 [Hanzo Studio](https://github.com/hanzoai/studio) 中 ControlNet 模型的安装与使用, 并完成一个涂鸦控制图像生成的示例。

![Hanzo Studio ControlNet 工作流](https://raw.githubusercontent.com/hanzoui/example-workflows/main/controlnet/scribble_controlnet.png)
...
![参考图](https://github.com/Fannovel16/hanzo_studio_controlnet_aux/blob/main/examples/CNAuxBanner.jpg?raw=true)

> 图源：[Hanzo Studio ControlNet aux](https://github.com/Fannovel16/hanzo_studio_controlnet_aux)
...
## Hanzo Studio ControlNet 工作流示例讲解

### 1. ControlNet 工作流素材

请下载下面的工作流图片,并拖入 Hanzo Studio 以加载工作流

![Hanzo Studio 工作流 - ControlNet](https://raw.githubusercontent.com/hanzoui/example-workflows/main/controlnet/scribble_controlnet.png)

<Tip>
Metadata 中包含工作流 json 的图片可直接拖入 Hanzo Studio 或使用菜单 `Workflows` -> `Open（ctrl+o）` 来加载对应的工作流。
该图片已包含对应模型的下载链接，直接拖入 Hanzo Studio 将会自动提示下载。
</Tip>

请下载下面的图片，我们将会将它作为输入

![Hanzo Studio 涂鸦图片](https://raw.githubusercontent.com/hanzoui/example-workflows/main/controlnet/scribble_input.png)
...
![Hanzo Studio 工作流 - ControlNet 流程图](/images/tutorial/controlnet/flow_diagram_scribble.png)
...
位于`Hanzo Studio\models\controlnet` 的模型会被 Hanzo Studio 检测到，并在这个节点中识别并加载
...
3. 访问 [ControlNet-v1-1_fp16_safetensors](https://huggingface.co/hanzoai/ControlNet-v1-1_fp16_safetensors/tree/main)  仓库下载其它类型的 ControlNet 模型，并尝试使用它们生成图像
