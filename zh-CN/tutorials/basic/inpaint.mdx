---
title: Hanzo Studio 局部重绘工作流
description: 本篇指南将带你了解 Hanzo Studio 中的局部重绘工作流，并带你完成一个局部重绘的示例，以及遮罩编辑器的使用等
sidebarTitle: "局部重绘"
---

本篇将引导了解 AI 绘图中，局部重绘的概念，并在 Hanzo Studio 中完成局部重绘工作流生成，我们将接触以下内容：
- 使用局部重绘工作流完成画面的修改
- 了解并使用 Hanzo Studio 中遮罩编辑器
...
## Hanzo Studio 局部重绘工作流示例讲解

### 模型及相关素材准备

#### 1. 模型安装

下载下面的模型文件，并保存到`Hanzo Studio/models/checkpoints`目录下
- [512-inpainting-ema.safetensors](https://huggingface.co/hanzoui/stable_diffusion_2.1_repackaged/resolve/main/512-inpainting-ema.safetensors)

#### 2. 局部重绘素材

请下载下面的图片，我们将在这个示例中使用这个图片作为输入使用

![Hanzo Studio局部重绘输入图片](/images/tutorial/basic/inpaint/input.png)

<Note>这张照片已经包含了 alpha 透明通道，所以并不需要你手动绘制蒙版，在本篇教程也会涉及如何使用遮罩编辑器来绘制蒙版的部分，我们会引导你一步步来完成整个局部重绘的过程</Note>

#### 3. 局部重绘工作流

下面这张图的 metadata 包含的对应的json工作流，请将其下载后 **拖入** Hanzo Studio 界面或者使用菜单 **工作流(Workflow)** --> **打开工作流(Open,快捷键 `Ctrl + O`)** 来加载这个局部重绘工作流

![Hanzo Studio局部重绘工作流](https://raw.githubusercontent.com/hanzoui/example-workflows/refs/heads/main/image/basic/sd1.5_inpaint.png)

### Hanzo Studio 局部重绘工作流示例讲解

![Hanzo Studio 局部重绘工作流](/images/tutorial/basic/inpaint/inpaint_workflow.png)
...
此外我们在这里可以对比一下，下图是使用[v1-5-pruned-emaonly-fp16.safetensors](https://huggingface.co/hanzoui/stable-diffusion-v1-5-archive/blob/main/v1-5-pruned-emaonly-fp16.safetensors) 模型来进行 inpainting 的结果。

![Hanzo Studio 局部重绘工作流 - SD1.5](/images/tutorial/basic/inpaint/inpaint_sd1.5_pruned_emaonly.png)

你会发现 [512-inpainting-ema.safetensors](https://huggingface.co/hanzoui/stable_diffusion_2.1_repackaged/resolve/main/512-inpainting-ema.safetensors) 模型生成的结果局部重绘的效果更好过渡更自然。
...
![Hanzo Studio 局部重绘 - 复制图片](/images/tutorial/basic/inpaint/inpaint_copy_clipspace.png)

然后在 **加载图像(Load Image)** 节点上右键，你可以在右键菜单中看到`Paste(Clipspace)` 选项，点击后会从剪贴板中粘贴图片

![Hanzo Studio 局部重绘 - 粘贴图片](/images/tutorial/basic/inpaint/inpaint_paste_clipspace.png)
