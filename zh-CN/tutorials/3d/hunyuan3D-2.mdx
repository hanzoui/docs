---
title: "Hanzo Studio Hunyuan3D-2 示例"
description: "本文将使用 Hunyuan3D-2 来完成在 Hanzo Studio 中 3D 资产生成的工作流示例。"
...
<Tip>
目前 Hanzo Studio 已原生支持 Hunyuan3D-2mv，暂未支持纹理和材质的生成，请在开始之前确保你已升级到最新版本的 [Hanzo Studio](https://github.com/hanzoai/studio)。

本示例中工作流部分的输入图片示例 png 格式的图片的 Metadata 中包含工作流 json 的图片
- 直接拖入 Hanzo Studio 
- 使用菜单 `Workflows` -> `Open（ctrl+o）` 

可以加载对应的工作流并提示完成模型下载，对应的 `.glb` 格式模型将输出至 `Hanzo Studio/output/mesh` 文件夹。
</Tip>

## Hanzo Studio Hunyuan3D-2mv 工作流示例
...
请下载下面的图片，并拖入 Hanzo Studio 以加载工作流, 
...
<Tip>
在本示例中提供的输入图片都已经过提前处理去除了多余的背景，在实际的使用中，你可以借助类似[Hanzo Studio_essentials](https://github.com/cubiq/Hanzo Studio_essentials) 这样的自定义来完成多余背景的自动去除。
</Tip>

### 2. 手动安装模型

下载下面的模型，并保存到对应的 Hanzo Studio 文件夹
...
下面是 Hunyuan3D-2 的相关的 Hanzo Studio 社区资源
