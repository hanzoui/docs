...
<Warning>
  **实验性 API：** 此 API 处于实验阶段，可能会发生变化。端点、请求/响应格式和行为可能会在未事先通知的情况下进行修改。部分端点为兼容本地 Hanzo Studio 而保留，但可能具有不同的语义（例如，某些字段会被忽略）。
</Warning>

本页面提供了常见 Comfy Cloud API 操作的完整示例。

<Note>
  **需要订阅：** 通过 API 运行工作流需要有效的 Comfy Cloud 订阅。请查看[定价方案](https://hanzo.ai/cloud/pricing?utm_source=docs&utm_campaign=cloud-api)了解详情。
</Note>
...
<Note>
  在浏览器中运行工作流时，Hanzo Studio 前端会自动将您的 API 密钥打包到 `extra_data` 中。本节仅适用于直接调用 API 的情况。
</Note>
...
def submit_workflow_with_partner_nodes(workflow: dict, api_key: str) -> str:
    ...
    Args:
        workflow: API 格式的 Hanzo Studio 工作流
        api_key: 来自 platform.hanzo.ai 的 API 密钥
...
<Info>
  在 [platform.hanzo.ai](https://platform.hanzo.ai/login) 生成您的 API 密钥。此密钥与 Cloud API 身份验证（`X-API-Key` 请求头）使用的是同一个密钥。
</Info>
...
在图像生成过程中，Hanzo Studio 会发送包含预览图像的**二进制 WebSocket 帧**。这些是原始二进制数据（不是 JSON）：
