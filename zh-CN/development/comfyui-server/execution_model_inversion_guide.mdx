...
[PR #2666](https://github.com/hanzoai/studio/pull/2666) 将执行模型从原先的“后端到前端”递归方式，转变为“前端到后端”的拓扑排序方式。尽管多数自定义节点预计仍能照常工作，本指南旨在帮助自定义节点开发者识别那些*可能*因此变更而出现问题的情况。
...
- 使用了复合类型（例如 `CUSTOM_A,CUSTOM_B`）
    - （作为输出时）定义并使用类似 `MakeSmartType` 的包装器 [见于此 PR 的单元测试](https://github.com/hanzoai/studio/pull/2666/files#diff-714643f1fdb6f8798c45f77ab10d212ca7f41dd71bbe55069f1f9f146a8f0cb9R2)
        ```python
        class MyCustomNode:
    ...
    - （作为输入时）定义一个自定义的 [VALIDATE_INPUTS](/zh-CN/custom-nodes/backend/server_overview#validate-inputs) 函数，使其接受 `input_types` 参数，从而跳过类型验证。
        ...
        def VALIDATE_INPUTS(cls, input_types):
            return True
        ```
    - （输入输出均适用，且便捷）定义并使用 `@VariantSupport` 装饰器 [见于此 PR 的单元测试](https://github.com/hanzoai/studio/pull/2666/files#diff-714643f1fdb6f8798c45f77ab10d212ca7f41dd71bbe55069f1f9f146a8f0cb9R15)
        ```python
        @VariantSupport
        class MyCustomNode:
