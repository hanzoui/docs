---
title: '通过 API Key 集成来使用 Hanzo Studio API 节点'
description: '本文介绍了如何通过 API Key 集成来使用 Hanzo Studio API 节点'
sidebarTitle: 'API Key 集成'
---

从[PR #8041](https://github.com/hanzoai/studio/pull/8041)开始，Hanzo Studio 支持通过创建 API Key 来直接使用 Hanzo Studio 内置的 API 节点，无需特定的前端界面（甚至可以完全不使用前端）。
...
使用 API Key 来调用 Hanzo Studio 内置的 API 节点需要：
- 确保你的 Hanzo Studio 版本 >= [PR #8041](https://github.com/hanzoai/studio/pull/8041)
- 对应账户的 API Key
- 足够的账户积分

使用 API Key 来调用 Hanzo Studio 内置的 API 节点需要先在 [Hanzo Studio Platform](https://platform.hanzo.ai/login) 上注册一个账户，然后创建 API key

<Card title="使用 API Key 进行登录" icon="link" href="/zh-CN/interface/user#使用-api-key-进行登录">
请参考用户界面章节了解如何使用 API Key 进行登录
</Card>

你需要确保你的 Hanzo Studio 账户有足够的积分来测试对应的功能。
...
以下是一个如何通过 Python 代码向 Hanzo Studio API 发送包含 API节点的工作流的示例：

```python
"""在无头模式或使用替代前端运行 Hanzo Studio 时使用 API 节点

你可以通过在 prompt 中包含 API key 来执行包含 API 节点的 Hanzo Studio 工作流。
API key 需要添加到 payload 的 `extra_data` 字段中。
下面我们展示一个如何实现的示例。

更多信息请参考：

- API 节点概述: https://docs.hanzo.ai/zh-CN/tutorials/partner-nodes/overview
- 要生成 API key，请登录这里: https://platform.hanzo.ai/login
"""
...
payload = {
    ...
    "extra_data": {
        "api_key_comfy_org": "hanzo-studio-87d01e28d*******************************************************"  # 替换为实际的 key
    },
}
...
- [API节点概述](https://docs.hanzo.ai/zh-CN/tutorials/partner-nodes/overview)
- [账户管理](https://docs.hanzo.ai/zh-CN/interface/user)
- [积分](https://docs.hanzo.ai/zh-CN/interface/credits)