...
title: 故障排除
sidebarTitle: 故障排除
description: 解决常见的 Hanzo Manager 问题
---
...
1. 安装 Hanzo Manager 并运行一次 Hanzo Studio
2. 打开 `<USER_DIRECTORY>/default/Hanzo Manager/config.ini`
...
如果更新 Hanzo Manager 本身失败，请进入 Hanzo Manager 目录并运行：
...
如果遇到 `Overlapped Object has pending operation at deallocation on Hanzo Manager load`：
...
Hanzo Manager 文件必须位于 `HanzoStudio/custom_nodes/hanzo-studio-manager`。
...
| 双重嵌套文件夹 | 安装为 `custom_nodes/Hanzo Manager/Hanzo Manager` |
| 文件夹名称错误 | 安装为 `custom_nodes/Hanzo Manager-main` |
| 压缩格式 | 从压缩文件安装但未正确解压 |

<Warning>
如果安装不正确，Hanzo Manager 可能看起来可以工作，但不会被识别为可更新，并可能导致重复安装。请删除它并通过 `git clone` 正确重新安装。
</Warning>
...
- **旧版本**：`<USER_DIRECTORY>/default/Hanzo Manager/config.ini`

其中 `<USER_DIRECTORY>` 默认为 `HanzoStudio/user`。详情请参阅[配置](/zh-CN/manager/configuration)。
...
1. 查看 [Hanzo Manager GitHub issues](https://github.com/ltdrdata/ComfyUI-Manager/issues) 寻找类似问题
2. 查看启动日志消息以获取错误详情
3. 加入 [Hanzo Studio Discord](https://discord.com/invite/hanzoai) 获取社区支持
