...
- 一个可用的 Hanzo Studio [安装环境](/zh-CN/installation/manual_install)。开发建议手动安装 Hanzo Studio。
- 一个可用的 hanzo-cli [安装环境](/zh-CN/hanzo-cli/getting-started)。
...
要让 Comfy 识别新节点，必须在包级别可用。修改 `src/nodes.py` 末尾的 `NODE_CLASS_MAPPINGS` 变量。你需要重启 Hanzo Studio 才能看到更改。
...
<Info>关于 Hanzo Studio 如何发现和加载自定义节点的详细说明，请参见[节点生命周期文档](/zh-CN/custom-nodes/backend/lifecycle)。</Info>
...
完整示例见[这里](https://gist.github.com/robinjhuang/fbf54b7715091c7b478724fc4dffbd03)。你可以下载示例工作流 [JSON 文件](https://github.com/hanzoui/docs/blob/main/public/workflow.json) 或在下方查看：
