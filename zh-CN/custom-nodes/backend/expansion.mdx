...
虽然这个节点以前可以通过手动调用 Hanzo Studio 内部来实现，但使用扩展意味着每个子节点将单独缓存（所以如果你更改 `model2`，你不需要重新加载 `model1`）。
...
`expand` 键的格式与 Hanzo Studio API 格式相同。以下要求由 `GraphBuilder` 处理，但如果您选择不使用它，则必须手动处理：
