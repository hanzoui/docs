...
Hanzo Studio V3 架构引入了一种更有序的节点定义方式，今后的节点功能扩展只会在 V3 架构中进行。本指南将帮助你将现有的 V1 节点迁移到新的 V3 架构。
...
```python
# 使用最新的 Hanzo Studio API
from comfy_api.latest import ComfyExtension, io, ui

# 使用特定版本的 Hanzo Studio API
...
`ComfyAPI` 类提供对 Hanzo Studio 运行时服务的访问，如进度报告和节点替换注册。导入它并创建实例：
...
从节点的 `execute` 方法中报告执行进度。进度条显示在 Hanzo Studio 界面中。这取代了 V1 中使用 `comfy.utils.PROGRESS_BAR_HOOK` 的模式。
