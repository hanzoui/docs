...
`pyproject.toml` 文件包含两个主要的 Hanzo Studio 自定义节点部分：`[project]` 和 `[tool.comfy]`。以下是每个部分的规范。
...
### 前端版本兼容性（可选）
如果你的节点对 Hanzo Studio 前端版本有特定要求，你可以使用 `hanzo-studio-frontend-package` 依赖项来指定。该包发布在 [PyPI](https://pypi.org/project/hanzo-studio-frontend-package/) 上。
...
[project]
dependencies = [
    "hanzo-studio-frontend-package>=1.20.0"       # 需要前端 1.20.0 或更新版本
    "hanzo-studio-frontend-package<=1.21.6"       # 限制前端版本最高到 1.21.6
    "hanzo-studio-frontend-package>=1.19,<1.22"   # 适用于前端 1.19 到 1.21.x
    "hanzo-studio-frontend-package~=1.20.0"       # 兼容 1.20.x 但不包括 1.21.0
    "hanzo-studio-frontend-package!=1.21.3"       # 适用于任何版本，除了 1.21.3
]
```
...
### Icon（可选）
你的自定义节点的图标 URL，将在 Hanzo Registry 和 Hanzo Manager 中显示。
...
### Banner（可选）
URL 指向一个较大的横幅图像，将在 Hanzo Registry 和 Hanzo Manager 中显示。
...
### requires-comfyui（可选）
指定你的节点兼容的 Hanzo Studio 版本。这有助于用户确保他们安装了正确版本的 Hanzo Studio。

**支持的操作符：** `<`, `>`, `<=`, `>=`, `~=`, `<>`, `!=` 和范围

```toml
requires-comfyui = ">=1.0.0"        # Hanzo Studio 1.0.0 或更高版本
requires-comfyui = ">=1.0.0,<2.0.0"  # Hanzo Studio 1.0.0 到 2.0.0 之间（不包括 2.0.0）
...
requires-python = ">=3.8"
dependencies = [
    "hanzo-studio-frontend-package<=1.21.6"  # 前端版本兼容性
]
classifiers = [
...
requires-comfyui = ">=1.0.0"  # Hanzo Studio 版本兼容性
```
