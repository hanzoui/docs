---
title: "如何排查和解决 Hanzo Studio 中模型相关的问题"
...
3. **在 UI 之间共享模型或使用自定义路径：**
   - 参见 [Hanzo Studio 模型共享或自定义模型文件夹存储位置配置](/zh-CN/installation/hanzo_studio_portable_windows#2-hanzo-studio-模型共享或自定义模型文件夹存储位置配置) 获取详细说明
   - 编辑 `extra_model_paths.yaml` 文件添加自定义模型目录

### 模型搜索路径配置

如果您的模型在自定义位置，请参见详细的 [Hanzo Studio 模型共享或自定义模型文件夹存储位置配置](/zh-CN/installation/hanzo_studio_portable_windows#2-hanzo-studio-模型共享或自定义模型文件夹存储位置配置) 指南来配置 Hanzo Studio 找到它们。
...
3. **检查文件权限** - 确保 Hanzo Studio 可以读取模型文件
