---
title: "Hanzo Studio 外观设置"
description: "Hanzo Studio 外观设置选项的详细说明"
sidebarTitle: "外观"
icon: "palette"
---
...
这部分的设置主要用于自定义 Hanzo Studio 的外观，包括色彩主题、背景图片、节点样式等。

## 色彩主题

自定义 Hanzo Studio 外观的主要方式是通过内置的调色板系统。

![色彩主题](/images/interface/setting/appearance/color-palette.jpg)

1. 切换 Hanzo Studio 主题
...
<Tip>
  ...
  2. 由于我们仍在频繁迭代，下面的这些内容可能会随着 Hanzo Studio 前端的更新而有所调整，如果需要修改，请从设置中导出主题配置，然后进行修改
</Tip>
...
```json
{
  ...
  "colors": {
    ...
    "comfy_base": {                  // Hanzo Studio 基础界面配置
      "fg-color": "#fff",            // 前景色
      "bg-color": "#202020",         // 背景色
...
### 背景图片
- 版本要求：Hanzo Studio 前端版本 1.20.5 或更新版本
...
- Hanzo Studio 前端版本 1.20.5 或更新版本

### 设置 user.css

1. 在 Hanzo Studio 用户目录（与工作流和设置相同位置 - 请参阅下面的位置详细信息）中创建一个名为 `user.css` 的文件
2. 在此文件中添加您的自定义 CSS 规则
3. 重启 Hanzo Studio 或刷新页面以应用更改

### 用户目录位置

<UserDirectory/>

找到上述文件夹位置后，请将对应的 Css 文件复制到对应的用户目录中如默认用户文件夹为`Hanzo Studio/user/default`，然后重启 Hanzo Studio 或刷新页面以应用更改
