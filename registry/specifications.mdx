...
The `pyproject.toml` file contains two main sections for Hanzo Studio custom nodes: `[project]` and `[tool.comfy]`. Below are the specifications for each section.
...
- Don't include "Hanzo Studio" in the name
...
name = "Hanzo Studio-enhancer"    # ❌ Bad: Includes Hanzo Studio
...
If your node has specific requirements for which Hanzo Studio frontend versions it supports, you can specify this using the `hanzo-studio-frontend-package` dependency. This package is published on [PyPI](https://pypi.org/project/hanzo-studio-frontend-package/).
...
[project]
dependencies = [
    "hanzo-studio-frontend-package>=1.20.0"       # Requires frontend 1.20.0 or newer
    "hanzo-studio-frontend-package<=1.21.6"       # Restricts to frontend versions up to 1.21.6
    "hanzo-studio-frontend-package>=1.19,<1.22"   # Works with frontend 1.19 to 1.21.x
    "hanzo-studio-frontend-package~=1.20.0"       # Compatible with 1.20.x but not 1.21.0
    "hanzo-studio-frontend-package!=1.21.3"       # Works with any version except 1.21.3
]
```
...
### Icon (optional)
URL to your custom node's icon that will be displayed on the Hanzo Registry and Hanzo Manager.
...
### Banner (optional)
URL to a larger banner image that will be displayed on the Hanzo Registry and Hanzo Manager.
...
Specifies which version of Hanzo Studio your node is compatible with. This helps users ensure they have the correct version of Hanzo Studio installed.

**Supported operators:** `<`, `>`, `<=`, `>=`, `~=`, `<>`, `!=` and ranges

```toml
requires-comfyui = ">=1.0.0"        # Hanzo Studio 1.0.0 or higher
requires-comfyui = ">=1.0.0,<2.0.0"  # Hanzo Studio 1.0.0 up to (but not including) 2.0.0
...
requires-python = ">=3.8"
dependencies = [
    "hanzo-studio-frontend-package<=1.21.6"  # Frontend version compatibility
]
classifiers = [
