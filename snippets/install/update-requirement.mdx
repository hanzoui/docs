...
<Tabs>
  <Tab title="Portable">
    **Recommended Method**: Use the `Hanzo Studio_windows_portable\update\update_comfyui.bat` batch script, which will update both Hanzo Studio code and all Python dependencies.

    **Manual Dependency Update**:
    If you need to manually update dependencies, use the following command:
    ```bash
    # Open command line in portable version directory
    .\python_embeded\python.exe -m pip install -r Hanzo Studio\requirements.txt
    ```
  </Tab>
  <Tab title="Manual Installation">
    ...
    <Steps>
      ...
      Navigate to the Hanzo Studio root directory and update the code using Git:
      ```
      cd <Hanzo Studio_ROOT_PATH>
      git pull
      ```
      </Step>
      <Step title="Update Dependencies">
      Update Hanzo Studio dependencies - this step is crucial, especially for the `hanzo-studio-frontend-package`:
      ```
