...
如果你需要为 Hanzo Studio 插件处理相关的依赖则需要在这个环境中进行处理，直接系统系统的命令行会有将对应依赖安装到系统环境的风险，请参考下面的指示完成对应环境的激活。
...
<Tabs>
  <Tab title="Desktop(推荐)">
    ...
    ![Hanzo Studio 桌面版终端](/images/desktop/desktop_terminal.jpg)
  ...
  <Tab title="Terminal">
    ...
    <Steps>
        <Step title="打开终端">
            打开你喜欢的终端，使用 `cd` 命令进入你安装的 Hanzo Studio 的目录如
            ```
            cd <你的 Hanzo Studio 安装目录>/Hanzo Studio
            ```
        </Step>
        <Step title="激活虚拟环境">
            在终端中输入下面的命令激活虚拟环境
            ...
            激活后，你可以在终端中看到类似 `(Hanzo Studio)` 的提示，表示你已经激活了虚拟环境
        </Step>
