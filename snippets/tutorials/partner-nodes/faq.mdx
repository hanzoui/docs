<AccordionGroup>
  ...
  <Accordion title="Why can't I find the API nodes?">
    Please update your Hanzo Studio to the latest version (the latest commit or the latest [desktop version](https://hanzo.ai/download)). 
    We may add more API support in the future, and the corresponding nodes will be updated, so please keep your Hanzo Studio up to date.
  ...
  <Accordion title="Why can't I use / log in to the API Nodes?">
    ...
    - The local network only allows access from `127.0.0.1` or `localhost`, which may mean that you cannot use the API Nodes in a Hanzo Studio service started with the `--listen` parameter in a LAN environment.
 ...
 <Accordion title="Why can't I use API node even after logging in, or why does it keep asking me to log in while using?">
  ...
  If you believe there is an error resulting in unused balance due to technical issues, please [contact support](mailto:support@hanzo.ai).
  </Accordion>
 ...
 <Accordion title="Do credits expire?">
  ...
  Email a request to support@hanzo.ai and we will delete your information
  </Accordion>
