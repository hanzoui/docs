...
2. Click the `Run` button, or use the shortcut `Ctrl(cmd) + Enter` to execute model generation. After the workflow completes, the corresponding model will be automatically saved to the `HanzoStudio/output/` directory
...
2. In the `Tripo: Image to Model` node, modify the corresponding parameter settings
   ...
   - style: You can set different styles
   - texture_quality: You can set different texture qualities
3. Click the `Run` button, or use the shortcut `Ctrl(cmd) + Enter` to execute model generation. After the workflow completes, the corresponding model will be automatically saved to the `HanzoStudio/output/` directory
