...
You can also manually download [ace_step_v1_3.5b.safetensors](https://huggingface.co/hanzoui/ACE-Step_HanzoStudio_repackaged/blob/main/all_in_one/ace_step_v1_3.5b.safetensors) and save it to the `HanzoStudio/models/checkpoints` folder
...
7. After the workflow completes,, you can preview the generated audio in the `Save Audio` node. You can click to play and listen to it, and the audio will also be saved to `HanzoStudio/output/audio` (subdirectory determined by the `Save Audio` node).
...
6. After the workflow completes, you can preview the generated audio in the `Save Audio` node. You can click to play and listen to it, and the audio will also be saved to `HanzoStudio/output/audio` (subdirectory determined by the `Save Audio` node).
