---
title: "Hanzo Studio ACE-Step Native Example"
description: "This guide will help you create dynamic music using the ACE-Step model in Hanzo Studio"
...
## ACE-Step Hanzo Studio Text-to-Audio Generation Workflow Example

### 1. Download Workflow and Related Models

Click the button below to download the corresponding workflow file. Drag it into Hanzo Studio to load the workflow information. The workflow includes model download information.

<a className="prose"  target='_blank' href="https://raw.githubusercontent.com/hanzoui/example-workflows/main/audio/ace-step/ace_step_1_t2m.json" style={{ display: 'inline-block', backgroundColor: '#0078D6', color: '#ffffff', padding: '10px 20px', borderRadius: '8px', borderColor: "transparent", textDecoration: 'none', fontWeight: 'bold'}}>
    <p className="prose" style={{ margin: 0, fontSize: "0.8rem" }}>Download Json Format Workflow File</p>
</a>

You can also manually download [ace_step_v1_3.5b.safetensors](https://huggingface.co/hanzoui/ACE-Step_Hanzo Studio_repackaged/blob/main/all_in_one/ace_step_v1_3.5b.safetensors) and save it to the `Hanzo Studio/models/checkpoints` folder
...
7. After the workflow completes,, you can preview the generated audio in the `Save Audio` node. You can click to play and listen to it, and the audio will also be saved to `Hanzo Studio/output/audio` (subdirectory determined by the `Save Audio` node).

## ACE-Step Hanzo Studio Audio-to-Audio Workflow
...
Click the button below to download the corresponding workflow file. Drag it into Hanzo Studio to load the workflow information.

<a className="prose"  target='_blank' href="https://raw.githubusercontent.com/hanzoui/example-workflows/main/audio/ace-step/ace_step_1_m2m_editing.json" style={{ display: 'inline-block', backgroundColor: '#0078D6', color: '#ffffff', padding: '10px 20px', borderRadius: '8px', borderColor: "transparent", textDecoration: 'none', fontWeight: 'bold'}}>
    <p className="prose" style={{ margin: 0, fontSize: "0.8rem" }}>Download Json Format Workflow File</p>
</a>

Download the following audio file as the input audio:

<a className="prose"   target='_blank' href="https://github.com/hanzoui/example-workflows/raw/refs/heads/main/audio/ace-step/input.mp3" style={{ display: 'inline-block', backgroundColor: '#0078D6', color: '#ffffff', padding: '10px 20px', borderRadius: '8px', borderColor: "transparent", textDecoration: 'none', fontWeight: 'bold'}}>
...
6. After the workflow completes, you can preview the generated audio in the `Save Audio` node. You can click to play and listen to it, and the audio will also be saved to `Hanzo Studio/output/audio` (subdirectory determined by the `Save Audio` node).
...
- In Hanzo Studio, we haven't fully implemented the conversion of all languages to English letters. Currently, only [Japanese hiragana and katakana characters](https://github.com/hanzoai/studio/commit/5d3cc85e13833aeb6ef9242cdae243083e30c6fc) are implemented.
...
<Note>
The language tags above have not been fully tested at the time of writing this documentation. If any language tag is incorrect, please [submit an issue to our documentation repository](https://github.com/hanzoui/docs/issues) and we will make timely corrections.
</Note>
