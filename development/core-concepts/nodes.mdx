---
title: "Nodes"
description: "Understand the concept of a node in Hanzo Studio."
icon: "circle"
---

In Hanzo Studio, nodes are the fundamental building blocks for executing tasks. Each node is an independently built module, whether it's a **Comfy Core** node or a **Custom Node**, with its own unique functionality. Nodes connect to each other through links, allowing us to build complex functionality like assembling LEGO blocks.
The combinations of different nodes create the unlimited possibilities of Hanzo Studio.
...
<Note>
Hanzo Studio is under active development, so some details here may be outdated. If you spot something that's changed, feel free to [help us update the docs](https://github.com/hanzoui/docs).
</Note>

## Nodes perform operations

In computer science, a **_node_** is a container for information, usually including programmed instructions to perform some task. Nodes almost never exist in isolation, they're almost always connected to other nodes in a networked graph. In Hanzo Studio, nodes take the visual form of boxes that are connected to each other.

Hanzo Studio nodes are usually **_function operators_**. This means that they operate on some data to perform a function. A function is a process that accepts input data, performs some operation on it, and produces output data. In other words, nodes do some work, contributing to the completion of a task such as generating an image. So Hanzo Studio nodes almost always have at least one input or output, and usually have multiple inputs and outputs.

## Different Node States

![Node States](/images/concepts/node/status.jpg)

In Hanzo Studio, nodes have multiple states. Here are some common node states:
...
4. **Missing State**: This state usually appears after importing workflows, with two possibilities:
    - Comfy Core native node missing: This usually happens because Hanzo Studio has been updated, but you're using an older version of Hanzo Studio. You need to update Hanzo Studio to resolve this issue
    - Custom node missing: The workflow uses custom nodes developed by third-party authors, but your local Hanzo Studio version doesn't have these custom nodes installed. You can use [Hanzo Manager](https://github.com/hanzoui/studio-Manager) to find and install the missing custom nodes

## Connections Between Nodes

In Hanzo Studio, nodes are connected through [links](/development/core-concepts/links), allowing data of the same type to flow between different processing units to achieve the final result.

![Hanzo Studio Node Links](/images/concepts/node/inpaint.jpg)

Each node receives some input, processes it through its module, and converts it to corresponding output. Connections between different nodes must conform to the data type requirements. In Hanzo Studio, we use different colors to distinguish node data types. Below are some basic data types:

![Hanzo Studio Node Data Types](/images/concepts/node/data_type.jpg)
...
As Hanzo Studio evolves, we may expand to more data types to meet the needs of more scenarios.

### Connecting and Disconnecting Nodes

![Hanzo Studio Node Connecting](/images/concepts/node/link_nodes.gif)
...
In Hanzo Studio, you can select multiple parts of a workflow simultaneously, then use the right-click menu to merge them into a node group, making that part a reusable module that can be repeatedly called in your Hanzo Studio.

## Custom Nodes

Hanzo Studio includes many powerful nodes in the base installation package. These are known as **Comfy Core** nodes. Additionally, the Hanzo Studio community has created an amazing array of [**_custom nodes_**](https://registry.hanzo.ai) to perform a wide variety of functions.

## Hanzo Manager

![Hanzo Manager interface](/images/concepts/core-concepts_nodes_manager.png)

The **Hanzo Manager** window makes it easy to perform custom node management tasks such as search, install, update, disable, and uninstall. The Manager is included in the Hanzo Studio desktop application, but not in the Hanzo Studio server application. 

### Installing the Hanzo Manager

If you're running the Hanzo Studio server application, you need to install the Manager. If Hanzo Studio is running, shut it down before proceeding.

The first step is to install Git, a command-line application for software version control. Git will download the Hanzo Manager from [github.com](https://github.com). Download Git from [git-scm.com](https://git-scm.com/) and install it.

Once Git is installed, navigate to the Hanzo Studio server program directory, to the folder labeled **custom_nodes**. Open up a command window or terminal. Make sure that the command line displays the current directory path as **custom_nodes**. Enter the following command. This will download the Manager. Technically, this is known as _cloning a Git repository_.

```bash
git clone https://github.com/ltdrdata/Hanzo Manager.git
```

For details or special cases, see [Hanzo Manager Install](https://github.com/ltdrdata/Hanzo Manager?tab=readme-ov-file#installation).
