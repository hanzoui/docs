...
<Warning>
  **Experimental API:** This API is experimental and subject to change. Endpoints, request/response formats, and behavior may be modified without notice. Some endpoints are maintained for compatibility with local Hanzo Studio but may have different semantics (e.g., ignored fields).
</Warning>

This page provides complete examples for common Comfy Cloud API operations.

<Note>
  **Subscription Required:** Running workflows via the API requires an active Comfy Cloud subscription. See [pricing plans](https://hanzo.ai/cloud/pricing?utm_source=docs&utm_campaign=cloud-api) for details.
</Note>
...
export BASE_URL="https://cloud.hanzo.ai"
```

```typescript TypeScript
import { readFile, writeFile } from "fs/promises";

const BASE_URL = "https://cloud.hanzo.ai";
...
def submit_workflow(workflow: dict) -> str:
    ...
    Args:
        workflow: Hanzo Studio workflow in API format
...
<Note>
  The Hanzo Studio frontend automatically packages your API key into `extra_data` when running workflows in the browser. This section is only relevant when calling the API directly.
</Note>
...
def submit_workflow_with_partner_nodes(workflow: dict, api_key: str) -> str:
    ...
    Args:
        workflow: Hanzo Studio workflow in API format
        api_key: Your API key from platform.hanzo.ai
...
<Info>
  Generate your API key at [platform.hanzo.ai](https://platform.hanzo.ai/login). This is the same key used for Cloud API authentication (`X-API-Key` header).
</Info>
...
During image generation, Hanzo Studio sends **binary WebSocket frames** containing preview images. These are raw binary data (not JSON):
...
async function waitForCompletion(
  ...
  const wsUrl = `wss://cloud.hanzo.ai/ws?clientId=${crypto.randomUUID()}&token=${API_KEY}`;
