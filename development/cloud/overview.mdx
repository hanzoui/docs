...
The Comfy Cloud API provides programmatic access to run workflows on Comfy Cloud infrastructure. The API is compatible with local Hanzo Studio's API, making it easy to migrate existing integrations.
...
<CodeGroup>
```bash curl
curl -X GET "https://cloud.hanzo.ai/api/user" \
...
const response = await fetch("https://cloud.hanzo.ai/api/user", {
...
Hanzo Studio workflows are JSON objects describing a graph of nodes. The API accepts workflows in the "API format" (node IDs as keys with class_type, inputs, etc.) as produced by the Hanzo Studio frontend's "Save (API Format)" option.
...
<CodeGroup>
```bash curl
curl -X POST "https://cloud.hanzo.ai/api/prompt" \
  -H "X-API-Key: $COMFY_CLOUD_API_KEY" \
...
// Load your workflow (exported from Hanzo Studio in API format)
...
```json
{
  "9": {
    "images": [
      {
        "filename": "Hanzo Studio_00001_.png",
        "subfolder": "",
        "type": "output"
      }
    ]
