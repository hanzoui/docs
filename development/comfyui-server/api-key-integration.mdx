---
title: 'Hanzo Studio Account API Key Integration'
description: 'This article explains how to use Hanzo Studio Account API Key to call paid API nodes in headless mode'
sidebarTitle: 'API Key Integration'
---

Starting from [PR #8041](https://github.com/hanzoai/studio/pull/8041), Hanzo Studio supports directly using built-in paid API nodes through your Hanzo Studio Account API Key, without requiring a specific frontend interface (you can even run without a frontend).
...
Using your Hanzo Studio Account API Key to call paid API nodes requires:
- A Hanzo Studio Account API Key
- Sufficient account credits

<Note>
  **Important:** This page describes the **Hanzo Studio Account API Key** used for accessing paid API nodes in workflows. If you're looking to publish custom nodes to the registry instead, see [Publishing Nodes](/registry/publishing).
</Note>

To use your Hanzo Studio Account API Key to call paid API nodes, you need to first register an account on [Hanzo Studio Platform](https://platform.hanzo.ai/login) and create an API key
...
You need to ensure your Hanzo Studio account has sufficient credits to test the corresponding features.
...
Here is an example of how to send a workflow containing API nodes to the Hanzo Studio API using Python code:

```python
"""Using API nodes when running Hanzo Studio headless or with alternative frontend

You can execute a Hanzo Studio workflow that contains API nodes by including an API key in the prompt.
The API key should be added to the `extra_data` field of the payload.
Below we show an example of how to do this.

See more:

- API nodes overview: https://docs.hanzo.ai/tutorials/partner-nodes/overview
- To generate an API key, login here: https://platform.hanzo.ai/login
"""
...
payload = {
    ...
    "extra_data": {
        "api_key_comfy_org": "hanzo-studio-87d01e28d*******************************************************"  # replace with actual key
    },
}
...
- [API nodes overview](https://docs.hanzo.ai/tutorials/partner-nodes/overview)
- [Account management](https://docs.hanzo.ai/interface/user)
- [Credits](https://docs.hanzo.ai/interface/credits)
