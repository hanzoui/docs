---
title: "Server Config"
description: "Detailed description of Hanzo Studio server configuration options"
icon: "server"
...
<Tip>
  Although we provide LAN listening settings for the Desktop version, as a desktop application, it is not suitable for use as a server. We recommend that if you need to use Hanzo Studio as a public service within the LAN, please refer to the manual deployment tutorial to deploy the corresponding Hanzo Studio service.
</Tip>
...
**Description**:
- When enabled, Hanzo Studio server logs and runtime information will not be displayed
