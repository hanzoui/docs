...
title: Configuration
sidebarTitle: Config
description: Configure Hanzo Manager settings
---
...
<Note>
If executed without any options, the `<USER_DIRECTORY>` path defaults to `Hanzo Studio/user`. It can be set using `--user-directory <USER_DIRECTORY>`.
</Note>
...
|----------|-------------|
| `COMFYUI_PATH` | Installation path of Hanzo Studio |
...
List pip spec requirements in `pip_auto_fix.list` (similar to `requirements.txt`). It will automatically restore specified versions when starting Hanzo Studio or when versions get mismatched during custom node installations. `--index-url` can be used.

### Use aria2 as downloader

For faster downloads, you can configure Hanzo Manager to use aria2. See the [aria2 setup guide](https://github.com/ltdrdata/Hanzo Manager/blob/main/docs/en/use_aria2.md).
...
Hanzo Manager provides `cm-cli`, a command line tool that allows you to use Manager features without running Hanzo Studio. This is useful for automating custom node installation and managing installations in headless environments.

For detailed cm-cli documentation, see the [official cm-cli guide](https://github.com/ltdrdata/Hanzo Manager/blob/main/docs/en/cm-cli.md).

For a more comprehensive CLI experience, consider using [hanzo-cli](/hanzo-cli/getting-started).
