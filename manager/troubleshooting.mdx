...
description: Resolve common Hanzo Manager issues
---
...
1. Install Hanzo Manager and run Hanzo Studio once
2. Open `<USER_DIRECTORY>/default/Hanzo Manager/config.ini`
...
### Hanzo Manager update fails

If updating Hanzo Manager itself fails, go to the Hanzo Manager directory and run:
...
If you encounter `Overlapped Object has pending operation at deallocation on Hanzo Manager load`:
...
Hanzo Manager files must be located in `HanzoStudio/custom_nodes/hanzo-studio-manager`.
...
| Double nested folder | Installed as `custom_nodes/Hanzo Manager/Hanzo Manager` |
| Wrong folder name | Installed as `custom_nodes/Hanzo Manager-main` |
| Compressed format | Installing from a compressed file without proper extraction |

<Warning>
If installed incorrectly, Hanzo Manager may appear to work but won't be recognized for updates and may cause duplicate installations. Remove it and reinstall properly via `git clone`.
</Warning>
...
- **Hanzo Studio v0.3.76+**: `<USER_DIRECTORY>/__manager/config.ini`
- **Older versions**: `<USER_DIRECTORY>/default/Hanzo Manager/config.ini`

Where `<USER_DIRECTORY>` defaults to `HanzoStudio/user`. See [Configuration](/manager/configuration) for more details.
...
1. Check the [Hanzo Manager GitHub issues](https://github.com/ltdrdata/ComfyUI-Manager/issues) for similar problems
2. Review the startup log messages for error details
3. Join the [Hanzo Studio Discord](https://discord.com/invite/hanzoai) for community support
